...
/**
 * BascomTask configuration. A standard {@link DefaultBascomConfig}
 * implementation is provided, which may be overridden and then set to replaced
 * whatever config is currently in place.
 * 
...
public interface IBascomConfig {
     ...
     * {@link com.ebay.bascomtask.main.Orchestrator#execute()} is called). Has
     * no effect when
     * {@link com.ebay.bascomtask.main.Orchestrator#execute(long)} is called, as
     * that method provides an explicit override of the default.
     * 
     * @return the default timeout in milliseconds
     * @see com.ebay.bascomtask.config.DefaultBascomConfig#getDefaultOrchestratorTimeoutMs()
     *      for system default value
     */
     ...
     * executing any task. Provides an opportunity to set the thread name, or
     ...
     * other tasks ready to execute. Provides an opportunity to clear any thread
     * state.
     * 
     * @param threadStat metadata for thread
     */
     ...
     * i.e. called once for each top-level invocation of
     * {@link com.ebay.bascomtask.main.Orchestrator#execute(long, String)}.
     * Subclasses can provide their own version instead of the default which
     * generates {@link TaskMethodClosure} instances directly. The returned
     * value can be a singleton or created for each call if the subclass needs
     * to keep its own state.
     * 
     * @param orc on which the execution was invoked
     * @param pass provided as param to
     *            {@link com.ebay.bascomtask.main.Orchestrator#execute(long, String)}
     * @return generator
     */
