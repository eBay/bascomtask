...
public class Loader {
    ...
    private void run2(ExecutorService es, int runLoop, int targetDurationSeconds, int threadsPerLoop,
            final Runner runner) throws Exception {

        report(runLoop,"Load test invoked with targetDurationSeconds=%d, threadsPerLoop=%d%n",targetDurationSeconds,
                threadsPerLoop);
        ...
        report(runLoop,"Over %d seconds with threadsPerLoop=%d loops=%d tput=%d avg=%.2fms xs=%d%n",
                targetDurationSeconds,threadsPerLoop,loops,throughput,avg,exceptions.intValue());
